...
const dataPath = path.join(__dirname, '..', '..', 'public', 'data', 'bookshelves', 'bookshelves_osm.json');
      ...
      res.on('end', () => {
        try {
          ...
          if (json.features && json.features.length > 0) {
            ...
            const street = props.street ? `${props.street}${props.housenumber ? ' ' + props.housenumber : ''}` : null;
...
async function enrich() {
  ...
  for (let i = 0; i < limit; i++) {
    const b = missing[i];
    process.stdout.write(`[${i+1}/${limit}] Geocoding ${b.id} (${b.lat}, ${b.lon})... `);
    ...
    try {
      ...
      if (result) {
        ...
        if (b.name === 'Public Bookshelf' && result.city) {
