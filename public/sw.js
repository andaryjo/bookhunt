...
self.addEventListener("install", (event) => {
  ...
  event.waitUntil(
    caches
      ...
      .then((cache) => {
        console.log(
          "[Service Worker] Pre-caching static app shell & CDN assets",
        );
        ...
        return Promise.allSettled(
          PRECACHE_LIST.map((url) => {
            return fetch(url)
              ...
              .catch((err) => {
                console.warn(
                  `[Service Worker] Skipping pre-cache for: ${url}`,
                  err,
                );
              });
          }),
...
async function limitCacheSize(cacheName, maxItems) {
  ...
  if (keys.length > maxItems) {
    ...
    console.log(
      `[Service Worker] Trimmed ${itemsToDelete} map tiles from cache`,
    );
  }
}
    ...
    event.respondWith(
      caches.open(DATA_CACHE).then((cache) => {
        return fetch(event.request)
          ...
          .catch(async () => {
            console.log(
              `[Service Worker] Offline fallback for data: ${requestUrl.pathname}`,
            );
            const cachedResponse = await cache.match(event.request);
            if (cachedResponse) {
              return cachedResponse;
            ...
            return new Response(
              JSON.stringify({ error: "Offline and data not cached" }),
              {
                status: 503,
